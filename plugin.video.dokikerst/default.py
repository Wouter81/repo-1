...
import utils, countdown, youtubezoeken
...
Kerstplaylist = 'http://addon.dokitv.nl/kerst/dokikerst.xml'    
Kerstradio = 'http://addon.dokitv.nl/kerst/Kerstmix.txt'    

addon = xbmcaddon.Addon('plugin.video.dokikerst')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.dokikerstMusicr3-%s]: %s" %(string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

                
def KerstIndex():
    addon_log("KerstIndex")
    getData(Kerstplaylist,'')
    addDir('Kerstmuziek Zoeken','',246,icon ,  fanart,'','','','')
    addDir('Radio','',72,icon,fanart,'','','','')
    addDir('Aftellen naar Kerst','',299,icon,fanart,'','','','')
    addDir('','','',icon,fanart,'','','','',Folder=False) 
    addDir('Informatie: Kerst Playlist','Privacy Policy',45,icon,fanart,'','','','')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

def RadioIndex():
    addon_log("RadioIndex")
    getData(Kerstradio,'')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    
def Privacy_Policy():
    text = ''
    twit = 'http://addon.dokitv.nl/kerst/tutorial.txt'
    ...
    for status, dte in match:
        ...
        text = text+dte+'\n'+status+'\n'+'\n'
    showText('[COLOR red][B]DOKI Kerst Playlist[/B][/COLOR]', text)
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('DOKI Kerst Select A Source', List)
         if rNo>=0:
             rName=str(List[rNo])
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('DOKI Kerst', 'Nothing Entered')
          return       
    
    Searchkey = Searchkey.lower()
    List=[]
    List.append(Kerstplaylist)
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('DOKI Kerst Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('DOKI Kerst','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('DOKI Kerst', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,Folder=True):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from DOKI Kerst Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
                                    
            if not name in FAV:
                contextMenu.append(('Add to DOKI Kerst Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if (not play_list) and not any(x in url for x in g_ignoreSetResolved) or not url.startswith("plugin://plugin.video.dokikerst"):#  (not url.startswith('plugin://plugin.video.f4mTester')):
            if regexs:
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from DOKI Kerst Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to DOKI Kerst Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if not has_bad_addon:
    if mode==None:
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    elif mode==18:
        addon_log("youtubedl")
        try:
            import youtubedl
        except Exception:
            xbmc.executebuiltin("XBMC.Notification(DOKI Kerst,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    elif mode == 299: countdown.Kerstcountdown()
