...
def List(url, page=None):
    ...
    xbmc.log(listhtml)
    match = re.compile(r'<span style="color: #004d86;">Nieuw bij HollandseHitsForum.nl</span></b></p>(.*?)<p style="text-align: center;" align="center"><b><span style="font-size: 9pt; color: #004d86;">Releases aanmelden:</span></b></p>', re.DOTALL | re.IGNORECASE).findall(listhtml)[0]
    match1 = re.compile(r'<td[^<]+<p><b><span[^>]+>(\d.*?)</span>.*?<td style="width: 5cm;.*?<span[^>]+>(.*?)</span>.*?<td[^>]+.*?<span[^>]+>(.*?)</span>', re.DOTALL | re.IGNORECASE).findall(match)
