...
import time
import random
import utils, xite, slamfm40, MMTop40
...
MusicBase = 'http://addon.dokitv.nl/musictv/musictv.txt'
RadioBase = 'http://addon.dokitv.nl/musictv/radio.txt'
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.musictvr3-%s]: %s" % string)
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(MusicTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(MusicTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def playLiveResolver(url, name):
    ...
    if resolved:
        ...
        xbmc.executebuiltin("XBMC.Notification(MusicTV,No playable link found. - ,5000)")  
...
def News():
    text = ''
    twit = 'https://raw.githubusercontent.com/MusicTV-2016/MusicTV/master/XML/Nieuws.xml'
    ...
    for status, dte in match:
        ...
        dte = '[COLOR yellow][B]'+dte+'[/B][/COLOR]'
        text = text+dte+'\n'+status+'\n'+'\n'
    showText('[COLOR yellow][B]News[/B][/COLOR]', text)



def Privacy_Policy():
    text = ''
    twit = 'https://raw.githubusercontent.com/MusicTV-2016/MusicTV/master/XML/informatie.xml'
    req = urllib2.Request(twit)
    req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')
    response = urllib2.urlopen(req)
    link=response.read()
    ...
    for status, dte in match:
        ...
        dte = '[COLOR yellow][B]'+dte+'[/B][/COLOR]'
        text = text+dte+'\n'+status+'\n'+'\n'
    showText('[COLOR yellow][B]Information[/B][/COLOR]', text)
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,isItFolder=True):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from MusicTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from MusicTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to MusicTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
