...
def List(url):
    listhtml = utils.getHtml2('http://xite.nl/videos/1')
    match = re.compile('<li class="span4 no-blur .*?<img src="(.*?)" />.*?<h3>(.*?)</h3>.*?<a href="(.*?)"><span>Bekijk video</span></a>', re.IGNORECASE | re.DOTALL).findall(listhtml)
    for img, name, videopage in match:
        name = utils.cleantext(name)
        videopage = "http://xite.nl" + videopage
        utils.addDownLink(name, videopage, 222, img, '', fanart=fanart)
#    if len(match) == 15:
#        npage = page + 1        
